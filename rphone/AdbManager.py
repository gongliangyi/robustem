...
class AdbManager():
    ...
    def adb_fastbot(self, times = "5", throttle="100"):
        logfile = self.logname
        packname = os.path.basename(self.packname).strip('.apk')
        cmd = "adb -d  shell CLASSPATH=/sdcard/monkeyq.jar:/sdcard/framework.jar exec app_process /system/bin com.android.commands.monkey.Monkey -p "+packname +" --agent robot --running-minutes "+times+" --throttle "+throttle+" --ignore-crashes -v -v -v --bugreport "+ " > ./XiaomiLogs/" + logfile
        return self.adb_cmd(cmd)
    ...
    def log_packname(self, level="W"):
        packname = os.path.basename(self.packname).strip('.apk')
        cmd = "adb -d  logcat -b events -b system -b main '*:" + level + "' --pid='adb -e shell pidof -s" + packname + "' -v time -v color -f " + self.logcat
        return self.adb_cmd(cmd)
    ...
    def log_delete(self):
        cmd = "adb -d  pull /sdcard/"+self.logcat+" ./XiaomiLogs/"+self.logcat
        self.adb_cmd(cmd)
    ...
    def fastbot_crashlog(self):
        packname = os.path.basename(self.packname).strip('.apk')
        cmd = "adb -d  pull /sdcard/crash-dump.log ./XiaomiLogs/"+packname+"_dump.txt"
        self.adb_cmd(cmd)
        cmd = "adb -d  pull /sdcard/oom-traces.log ./XiaomiLogs/"+packname+"_traces.txt"
        self.adb_cmd(cmd)
        cmd = "adb -d  shell rm -rf /sdcard/crash-dump.log /sdcard/oom-traces.log"
        self.adb_cmd(cmd)
